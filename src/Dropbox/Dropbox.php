...
class Dropbox
{
    ...
    public function __construct(DropboxApp $app, array $config = [])
    {
        ...
        $httpClient = DropboxHttpClientFactory::make($config['http_client_handler'], $config['cacert'] );
    ...
    {
        if(!get_site_option('buwd_drop_file')){
            //Make Dropbox File
            $dropboxFile = $this->makeDropboxFile($dropboxFile);
            update_site_option('buwd_drop_file', $dropboxFile);
        } else{
            $dropboxFile = get_site_option('buwd_drop_file');
        }
    ...
    public function simpleUpload($dropboxFile, $path, array $params = [])
    {
        ...
        delete_site_option('buwd_drop_file');
        //Make and Return the Model
        return new FileMetadata($body);
    }
    ...
    {
        $drop_job_object = Buwd_Destination_Dropbox::$drop_job_object;
       ...
       // $sessionId = $this->startUploadSession($dropboxFile->getFilePath(), $chunkSize);
        if(!isset($drop_job_object->steps_data[$drop_job_object->current_step]['sessionId'])){
            $drop_job_object->steps_data[$drop_job_object->current_step]['sessionId'] = $this->startUploadSession($dropboxFile->getFilePath(), $chunkSize);

            //Uploaded
            $drop_job_object->steps_data[$drop_job_object->current_step]['uploaded'] = $chunkSize;

            //Remaining
            $drop_job_object->steps_data[$drop_job_object->current_step]['remaining'] =  $fileSize - $chunkSize;
        ...
        while ($drop_job_object->steps_data[$drop_job_object->current_step]['remaining'] > $chunkSize) {
           
            $drop_job_object->restart_if_needed();

            //Append the next chunk to the Upload session
            $drop_job_object->steps_data[$drop_job_object->current_step]['sessionId'] = $this->appendUploadSession($dropboxFile, $drop_job_object->steps_data[$drop_job_object->current_step]['sessionId'], $drop_job_object->steps_data[$drop_job_object->current_step]['uploaded'], $chunkSize);

            //Update remaining and uploaded
            $drop_job_object->steps_data[$drop_job_object->current_step]['uploaded'] = $drop_job_object->steps_data[$drop_job_object->current_step]['uploaded'] + $chunkSize;
            $drop_job_object->steps_data[$drop_job_object->current_step]['remaining'] = $drop_job_object->steps_data[$drop_job_object->current_step]['remaining'] - $chunkSize;
        }
        delete_site_option('buwd_drop_file');
        //Finish the Upload Session and return the Uploaded File Metadata
        return $this->finishUploadSession($dropboxFile, $drop_job_object->steps_data[$drop_job_object->current_step]['sessionId'], $drop_job_object->steps_data[$drop_job_object->current_step]['uploaded'], $drop_job_object->steps_data[$drop_job_object->current_step]['remaining'], $path, $params);
    }
